...
<html>
	...
	<body>
		...
		<div id="bgStyle"></div>
			...
			<?php
				$carrents = "SELECT * FROM autokolcsonzes";
				$getCarrents = mysqli_query($conn, $carrents);
				$countCarrents = mysqli_num_rows($getCarrents);
				
				$sumPriceC = "SELECT sum(ar_osszesen) AS osszesar FROM autokolcsonzes";
				$sumPriceQueryC = mysqli_query($conn, $sumPriceC );
				$rowSumC = mysqli_fetch_assoc($sumPriceQueryC); 
				$sumC = $rowSumC['osszesar'];
			?>
			...
			<div align = "center" width = "85%">
				...
				-moz-border-radius: 0 0 12 12;">
					<tr>
						...
						<b>Autókölcsönzés törlése az adatbázisból <i class="fa fa-trash "></i></font> (Összbevétel: <?php echo $sumC; ?> Ft) 
						</td><td bgcolor = "#FE2E2E"style = "padding-left: 10px; font-size: 25px; border-radius:0 10 10 0;
						-webkit-border-radius: 0 10 10 0;	-moz-border-radius: 0 10 10 0; border-left: 1px solid white;"><?php echo $countCarrents; ?> db</td>
					</tr>
					<tr>
					...
					<?php
						$cars = "SELECT * from auto, autokolcsonzes, felhasznalo 
						WHERE auto.id = autokolcsonzes.auto_id AND felhasznalo.felhasznalo_nev = autokolcsonzes.felhasznalo_nev";
						...
						if (mysqli_num_rows($getCars) > 0){
							...
							while($row = mysqli_fetch_assoc($getCars)){
								echo '
									<tr height = "30px">
										...
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.$row["automarka_id"].'</td>
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.$row["marka_tipus"].'</td>
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.$row["vezetek_nev"].' '.$row["kereszt_nev"].'</td>
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.$row["mettol"].'</td>
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.$row["meddig"].' </td>
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.$row["ar_naponta"].' Ft</td>
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.$row["ar_osszesen"].' Ft</td>
										<td colspan = "2" height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.$row["felhasznalo_nev"].'</td>
									';
									?>
									
										<td>
											<form method = "POST" action = "carrentdelete_action.php" enctype = "multipart/form-data" name = "deletcarrents">
							...
							<tr height = "30px" bgcolor = "#FE2E2E" >
								...
								<i class="fa fa-times"></i> Nem szerepel autókölcsönzés az adatbázisban!</td>
							</tr>';
						}
					?>
				</table>
