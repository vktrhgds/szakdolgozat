			...
			<table id = "carmotortable" style="height: 100%; position: fixed;display: inline-block; width: 20%; margin-left: 5%; border-right: 1px solid #6E6E6E; background: #EAE9E9; color: black;" align = "center" cellspacing = "0" cellpadding = "0">
					...
					<tr>
						...
						<td colspan = "2" style = "text-align: left; padding-top: 0px;" width = "15%">
							<select style = "width: 95%;" name = "select_motortype">
								...
								<option>
									<?php echo htmlspecialchars($rows["motormarka_id"]); } ?>
								</option>
							</select>
						</td>
					</tr>
						...
						<td colspan = "2" style = "text-align: left; padding-top: 0px;" width = "15%">
							<select style = "width: 95%;" name = "select_motorcolor">
								...
								<option>
									<?php echo htmlspecialchars($rows["motorszin"]); } ?>
								</option>
							</select>
						</td>
					</tr>
				...
				<?php 
					if(isset($_GET['search_extended_button_motor'])){
						...
						$minprice_motor = trim($_GET['minprice_motor'], '\n\t');
						$minprice_motor = mysqli_real_escape_string($conn, $_GET['minprice_motor']);
						$minprice_motor = addslashes(strip_tags($minprice_motor));
						
						$maxprice_motor = trim($_GET['maxprice_motor'], '\n\t');
						$maxprice_motor = mysqli_real_escape_string($conn, $_GET['maxprice_motor']);
						$maxprice_motor = addslashes(strip_tags($maxprice_motor));
						
						$category_motor = trim($_GET['select_categ_motor'], '\n\t');
						$category_motor = mysqli_real_escape_string($conn, $_GET['select_categ_motor']);
						$category_motor = addslashes(strip_tags($category_motor));
						
						$motortype = trim($_GET['select_motortype'], '\n\t');
						$motortype = mysqli_real_escape_string($conn, $_GET['select_motortype']);
						$motortype = addslashes(strip_tags($motortype));
						
						$fuel_motor = trim($_GET['select_fuel_motor'], '\n\t');
						$fuel_motor = mysqli_real_escape_string($conn, $_GET['select_fuel_motor']);
						$fuel_motor = addslashes(strip_tags($fuel_motor));
						
						$enginecap_motor = trim($_GET['select_enginecap_motor'], '\n\t');
						$enginecap_motor = mysqli_real_escape_string($conn, $_GET['select_enginecap_motor']);
						$enginecap_motor = addslashes(strip_tags($enginecap_motor));
						
						$motorcolor = trim($_GET['select_motorcolor'], '\n\t');
						$motorcolor = mysqli_real_escape_string($conn, $_GET['select_motorcolor']);
						$motorcolor = addslashes(strip_tags($motorcolor));
					...
					if(isset($_GET['search_simple_button_motor'])){
						...
						$search_simple = trim($_GET['search_simple_motor'], '\n\t');
						$search_simple = mysqli_real_escape_string($conn, $_GET['search_simple_motor']);
						$search_simple = htmlspecialchars($search_simple);
