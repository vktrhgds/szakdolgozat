					...
					<form method = "get" action = "viewcar.php" enctype = "multipart/form-data" name = "search_simple">
						...
						<tr>
							<td colspan = "2"style = "text-align: left;" width = "15%">
								<a target = "_blank" href = "<?php echo htmlspecialchars($row["fenykep"]); ?>" title = "A következő megjelenítése új lapon: <?php echo htmlspecialchars($row["automarka_id"])." ".$row["marka_tipus"]; ?>">
							...
							<td colspan = "2"style = "text-align: left; padding-top: 10px;" width = "15%">
								<?php echo htmlspecialchars($row['automarka_id'])." ".htmlspecialchars($row['marka_tipus']).", ".htmlspecialchars($row['kategoria']); ?>
							</td>
						</tr>
						</tr>
				...
				if (mysqli_num_rows($cars) > 0){
					while($row = mysqli_fetch_assoc($cars)){
						...
						<div align = "center">
								...
								style = " border-radius: 0;z-index: 0; display: inline-block; width: 69.5%; margin-left: 20%; border-bottom: 6px solid #Dadada; background: #E6E6E6; color: black;" >
									<tr>
										<td style = "text-align: right; padding-right:12px;" width = "25%">Autómárka</td>
										<td style = "text-align: left; padding: right: 10px;" width = "15%"><b><?php echo htmlspecialchars($row['automarka_id']); ?></b></td>
										<td rowspan = "17" width = "60%">
										<a target = "_blank" href = "<?php echo htmlspecialchars($row["fenykep"]); ?>" title = "A következő megjelenítése új lapon: <?php echo $row["automarka_id"]." ".$row["marka_tipus"]; ?>">
									...
									<tr>
										...
										<td style = "text-align: left; padding-right: 10px;"><b><?php echo htmlspecialchars($row['km_ora_allasa']); ?> km</td>
									</tr>
									<tr>
										<td style = "text-align: right; padding-right:12px;"width = "25%">Szállítható személyek</td>
										<td style = "text-align: left; padding-right: 10px;"><b><?php echo htmlspecialchars($row['szallithato_szemelyek']); ?> személy</td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td style = "text-align: left; padding-right: 10px;"><b><?php echo htmlspecialchars($row['autoszin']); ?> </b></td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td style = "text-align: left; padding-right: 10px;"><b><?php echo htmlspecialchars($row['gyorsulas']); ?> mp</b></td>
									</tr>
									<tr>
										<td style = "text-align: right; padding-right:12px;">Oktánszám</td>
										<td style = "text-align: left; padding-right: 10px;"><b><?php echo htmlspecialchars($row['oktanszam']); ?> </b></td>
									</tr>
									<tr>
							...
							<form method = "POST" action = "ratecar">
							...
							<table cellspacing = "0" cellpadding = "0" id = "admintable" width = "92.95%"
								...
								background: rgb(220,220,220);">
									...
									<td width = "70%" style = "font-size: 19px; padding-bottom: 10px;" colspan = "2">
										<input type="radio" name="carstars" value = "1" id="star-1" checked hidden />
										<input type="radio" name="carstars" value = "2" id="star-2" hidden />
										<input type="radio" name="carstars" value = "3" id="star-3" hidden />
										<input type="radio" name="carstars" value = "4" id="star-4" hidden />
										<input type="radio" name="carstars" value = "5" id="star-5" hidden />
									...
									<tr>
										<td colspan = "2">
											<textarea rows = "3" name = "carcomment" style = "border: 1px solid #BDBDBD; width: 100%; padding: 10px; font-size: 17px; color: black; font-family: Arial;"
									...
									<tr>
										<td style = "padding-top: 0px;" colspan = "2">
											<input type = "submit" class= "delete_edit3" name = "carratebutton" style = "width: 25%; height: 28px; font-size: 20px;font-family: 'Raleway', serif;"
											value = "Elküldés"/>
										</td>
									</tr>
									<tr>
							...
							<?php
								...
								$specific_carrate = "SELECT * FROM autosertekelesek, felhasznalo where autosertekelesek.auto_id = '".$_SESSION["carrate_id"]."'
								AND autosertekelesek.felhasznalo_nev = felhasznalo.felhasznalo_nev ORDER BY ertekeles DESC;";
								$carrate_query = mysqli_query($conn, $specific_carrate);
								$countcarrates = mysqli_num_rows($carrate_query);
								...
								if ($countcarrates > 0){
									...
									<div align = "center">
										...
										margin-left: 20%; border-bottom: 6px solid #Dadada; background: white; color: black;" >
											<tr>
												<td>
													Értékelések (<?php echo htmlspecialchars($countcarrates); ?>)
												</td>
											</tr>
										</table>
									</div>
									...
									while($row = mysqli_fetch_assoc($carrate_query)){
										?>
										...
										<div align = "center">
											...
											<tr>
												...
												?></font>
												<font style = "font-size: 11px;"><?php echo htmlspecialchars($row['datum']); ?></font>
												</td>
											</tr>
											<tr>
												<td colspan = "2" style = " width: 100%; font-size: 14px; padding-left: 15px; padding-bottom: 15px;"> <!-- ide a komment maga -->
