		...
		<div id="bgStyle" style = "background: #EAE9E9;"></div>
			...
			<?php
				if(isset($_POST["updateprofile"])){
					...
					$irszam =  mysqli_real_escape_string($conn, $_POST['irszam']);
					...
					$szulhely =  mysqli_real_escape_string($conn, $_POST['szulhely']);
					$szulido =  mysqli_real_escape_string($conn, $_POST['szulido']);
					$anyjavnev =  mysqli_real_escape_string($conn, $_POST['anyjavnev']);
					$anyjaknev =  mysqli_real_escape_string($conn, $_POST['anyjaknev']);
					
					$checkPassWord = "SELECT * FROM felhasznalo WHERE felhasznalo_nev = '".$login_session."' AND jelszo = '".$password_hashed."';";
					$checkPassWordQuery = mysqli_query($conn, $checkPassWord);
					$checkPassCount = mysqli_num_rows($checkPassWordQuery);
						...
						die('</br></br></br>
							<div class="container">
								<div class="alert alert-danger">
									<strong>Sikertelen adatmódosítás! </strong><i class="fa fa-frown-o" aria-hidden="true"></i></br>
									Helytelenül adta meg jelenlegi jelszavát!</br>
									Ügyeljen a kis- és nagybetűkre, illetve ellenőrizze, hogy a jelszava legalább 8, de legfeljebb 32 karakterből áll!
								</div>
