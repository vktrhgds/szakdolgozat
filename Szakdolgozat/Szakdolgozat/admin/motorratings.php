...
<html>
	...
	<body>	
		<div id="bgStyle"></div>
			...
			<?php 
				$AllMotorRatings = "SELECT * FROM motor, motorosertekelesek, felhasznalo WHERE 
				motorosertekelesek.motor_id = motor.id AND felhasznalo.felhasznalo_nev
				= motorosertekelesek.felhasznalo_nev ORDER BY ertekeles DESC;";
				$AllMotorRatingsQuery = mysqli_query($conn, $AllMotorRatings);
				$countAllMotorRatings = mysqli_num_rows($AllMotorRatingsQuery);
				
				if($countAllMotorRatings > 0){
				?>
					...
					<div align = "center" width = "76.5%" style = "z-index: -1;" >
						<table cellspacing = "0" cellpadding = "0" id = "admintable" align = "center" width = "81.5%"
								...
								z-index: 0;">
									<tr>
										<td colspan = "2" style = "padding-bottom: 35px; padding-left: 10px; font-size: 35px;" >Motoros értékelések (<?php echo $countAllMotorRatings; ?>)</td> <!-- ide a darabszám -->
									</tr>
									
									<?php while($rowMR = mysqli_fetch_assoc($AllMotorRatingsQuery)){
									?>
									
									<tr>
										<td style = "font-size: 20px; padding: 10px; width: 90%"><font color = "yellow"><b><?php echo htmlspecialchars($rowMR['felhasznalo_nev']); ?></b></font>  értékelése:
										<font color = "yellow">
										<?php 
												if(htmlspecialchars($rowMR['ertekeles']) == 1){
													?> <i class="fa fa-star" style = "color: #FF0000;"></i> <?php
												}
												else if(htmlspecialchars($rowMR['ertekeles']) == 2){
													?> <i class="fa fa-star" style = "color: #FF4000;"></i> <i class="fa fa-star" style = "color: #FF4000;"></i> <?php
												}
												else if(htmlspecialchars($rowMR['ertekeles']) == 3){
												...
												else if(htmlspecialchars($rowMR['ertekeles']) == 4){
												...
												else if(htmlspecialchars($rowMR['ertekeles']) == 5){
										...
										?>
										</font><font color = "white"><?php echo htmlspecialchars($rowMR['datum']); ?> dátummal </font> 
										<font color = "yellow">(<?php echo htmlspecialchars($rowMR['motormarka_id'])." ".htmlspecialchars($rowMR['marka_tipus']); ?>)</font>
										</td>
										<td style = "width: 10%;">
											<form method = "post" action = "motorratings_delete">
												<?php echo "<input type = 'submit' class = 'delete_edit2' style = 'width: 100%; ' value = 'Törlés' name = ".$rowMR['id'].">"?>
											</form>
										</td>
									</tr>
									<tr>
										<td colspan = "2" style = "font-size: 15px; padding: 10px;"> <!-- ide a komment maga -->
