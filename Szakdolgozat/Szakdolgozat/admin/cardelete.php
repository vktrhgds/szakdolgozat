...
<html>
	...
	<body>
		...
		<div id="bgStyle"></div>
			...
			<?php
						$cars = "SELECT * FROM auto ORDER BY kategoria DESC";
						$getCars = mysqli_query($conn, $cars);
						
						$allcars = "SELECT * from auto";
						$carResult = mysqli_query($conn, $allcars);
						$count_car = mysqli_num_rows($carResult);
			?>
			...
			<div align = "center" width = "85%">
				...
				-moz-border-radius: 0 0 12 12;">
					<tr>
						...
						<b>Személygépkocsi törlése az adatbázisból <i class="fa fa-trash "></i></font>
						</td><td bgcolor = "#FE2E2E"style = "padding-left: 10px; font-size: 25px; border-radius:0 10 10 0;
						-webkit-border-radius: 0 10 10 0;	-moz-border-radius: 0 10 10 0; border-left: 1px solid white;"><?php echo $count_car; ?> db</td>
					</tr>
					<tr>
						...
						if (mysqli_num_rows($getCars) > 0){
							...
							echo '
								<tr bgcolor = "#6E6E6E" >
									...
									<td height = "25px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 19px;"><b>Száll. személyek</b></td>
							...
							while($row = mysqli_fetch_assoc($getCars)){
								echo '
									<tr height = "30px">
										...
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.htmlspecialchars($row["automarka_id"]).'</td>
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.htmlspecialchars($row["marka_tipus"]).'</td>
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.htmlspecialchars($row["evjarat"]).'</td>
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.htmlspecialchars($row["allapot"]).'</td>
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.htmlspecialchars($row["km_ora_allasa"]).' km</td>
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.htmlspecialchars($row["szallithato_szemelyek"]).' személy</td>
										<td height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.htmlspecialchars($row["uzemanyag"]).'</td>
										<td colspan = "2" height = "20px" align = "left" style =" padding: 5px 0px 0px 5px; font-size: 15px;">'.htmlspecialchars($row["teljesitmeny"]).' LE</td>
									';
									?>
									
										<td>
											<form method = "get" action = "cardelete_action.php" enctype = "multipart/form-data" name = "deletecar">
							...
							<tr height = "30px" bgcolor = "#FE2E2E" >
								...
								<i class="fa fa-times"></i> Nem szerepel személygépkocsi az adatbázisban!</td>
							</tr>';
						}
